...
async function test() {
  ...
  // Read actual product image to base64
  const imgData = fs.readFileSync('public/products/luz-pisca-pisca.webp');
  const imgBase64 = "data:image/webp;base64," + imgData.toString('base64');
  ...
  const payload = {
    ...
    enhancements: [],
    aspectRatio: "9:16",
    productImageUrl: imgBase64
  };

  console.log("Sending payload with real base64 product image...");
